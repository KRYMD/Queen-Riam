...
> **IMPORTANT (Baileys v7.0.0-rc.9):** Button taps do NOT arrive via
> `interactiveResponseMessage` as you might expect. They arrive as
> `templateButtonReplyMessage.selectedId`. The `selectedId` contains
> the `id` string you passed when creating the button — no JSON parsing needed.
...
> **Note on list messages:** As of Baileys v7.0.0-rc.9, list messages
> (`sendInteractiveMessage` with `list` type) do not render on most
> WhatsApp clients. Stick to quick-reply buttons for interactive menus.
